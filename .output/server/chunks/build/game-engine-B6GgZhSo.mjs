class GameEngine {
  ...
  generateMap(beatTimes, sections, duration, fixedSeed, verify = true, offsetAttempt = 0, bpm = 120, measureLength = 2) {
    ...
    for (let attempt = 0; attempt < maxRetries; attempt++) {
      ...
      this.autoplayLog = [];
      let rng = this.seededRandom(seed + currentAttempt);
  ...
  generatePatternBasedMap(beatTimes, sections, rng) {
    const { density, portalFrequency } = this.mapConfig;
    let currentTime = 0;
    this.mapConfig.difficulty >= 24;
    let currentX = 200;
    let currentMini = false;
    let currentInverted = false;
    let lastPatternEndTime = 0;
    let lastPortalX = 0;
    let lastRequiredY = "middle";
    let lastSpeedType = "speed_1";
    let lastPatternType = void 0;
    const measureTimes = [];
    ...
    for (const event of events) {
      ...
      if (event.type === "measure") {
        ...
        if (diff < 8) {
          ...
          else if (intensity < 0.65) portalType = "speed_2";
        ...
        if (diff < 8) {
          validPatterns = validPatterns.filter(
            (p) => !p.type?.includes("laser") && !p.type?.includes("moving") && !p.type?.includes("spike_ball") && !p.type?.includes("mine") && !p.type?.includes("orb")
          );
        }
        ...
        if (roll < 0.2) {
          ...
          const playH = this.maxY - this.minY;
          const types = ["orb", "mine", "saw", "mini_spike", "spike_ball"];
          for (let j = 0; j < count; j++) {
            ...
            this.obstacles.push({
              ...
              height: dw,
              type,
              initialY: this.minY + dy,
              movement: { type: "none", range: 0, speed: 0, phase: 0 }
            });
          }
        ...
        const patternsWithWeight = validPatterns.map((p) => {
          let weight = 1;
          const type = p.type || "";
          if (type.includes("mine") || type.includes("orb") || type.includes("cluster") || type.includes("field")) weight = 4;
          if (type.includes("mini_spike")) weight = 2.5;
          if (type === "square_block" || type === "corridor") weight = 0.25;
          return { p, weight };
        });
        ...
        if (currentX - lastPortalX > 800 * speedM && rng() < portalFrequency * 2.8) {
          const gravityType = rng() > 0.5 ? "gravity_yellow" : "gravity_blue";
  ...
  *computeAutoplayLogGen(startX, startY) {
    ...
    while (stack.length > 0) {
      ...
      const dH = checkColl(nX, nYH, sz, nT, 0);
  ...
  checkObstacleCollision(obs, px, py, pSize, simTime) {
    ...
    if (px + pSize <= obs.x - margin || px - pSize >= obs.x + obs.width + margin || py + pSize <= obsY - margin || py - pSize >= obsY + obs.height + margin) {
      return false;
    }
    const points = [
    ...
    if (obs.type === "laser") {
      ...
      const centerY = obsY + obs.height / 2;
      return px + pSize > obs.x && px - pSize < obs.x + obs.width && py + pSize > centerY - laserH && py - pSize < centerY + laserH;
    }
    if (obs.type === "v_laser") {
      const laserW = obs.width * 0.4;
      const centerX = obs.x + obs.width / 2;
      return py + pSize > obsY && py - pSize < obsY + obs.height && px + pSize > centerX - laserW && px - pSize < centerX + laserW;
    }
    if (obs.type === "spike" || obs.type === "mini_spike") {
      ...
      if (isBottomSpike) {
        for (const p of points) {
          if (this.isPointInTriangle(
            ...
            p.y,
            obs.x + padding,
            obsY + obs.height - padding,
            centerX,
            obsY + padding,
            obs.x + obs.width - padding,
            obsY + obs.height - padding
          )) return true;
        }
      } else {
    ...
    return px + pSize > obs.x && px - pSize < obs.x + obs.width && py + pSize > obsY && py - pSize < obsY + obs.height;
  }
...
//# sourceMappingURL=game-engine-B6GgZhSo.mjs.map
